...
class ExampleTest extends TestCase
{
    ...
    public function test_browser_admin_requires_login_and_customer_otp_routes_stay_disabled(): void
    {
        ...
        $this->get('/admin/login')
            ...
            ->assertDontSee('Enter Your Phone Number');
        $this->get('/apps')->assertRedirect('/login');
    }

    public function test_admin_can_use_pin_and_authenticator_to_open_shared_admin(): void
    {
        ...
        $this->postJson('/admin/auth/login', ['phone' => '9999999999', 'pin' => '123456', 'code' => $code])
            ->assertOk()
            ->assertJsonPath('redirect', route('admin.dashboard'));

        $this->assertAuthenticatedAs($admin);
        $this->get('/admin')->assertOk()->assertSee('Web admin');
    }

    public function test_sale_route_only_returns_sale_products(): void
    {
        Storage::fake('products');
        foreach (['regular', 'sale'] as $type) {
            $product = Product::query()->create([
                'name' => ucfirst($type).' Suit',
                'slug' => $type.'-suit',
                'sku' => strtoupper($type),
                'product_type' => $type,
                'price' => 500,
            ...
            $product->images()->create([
                'disk' => 'products', 'path' => $type.'.jpg', 'original_name' => $type.'.jpg',
                'sort_order' => 0, 'is_cover' => true,
            ]);
        }

        $this->get('/sale')->assertOk()->assertSee('Sale Catalog');
        $this->getJson('/products?product_type=sale')
            ->assertOk()
            ->assertJsonCount(1, 'data')
            ->assertJsonPath('data.0.product_type', 'sale');
    }
    ...
    public function test_wordpress_import_dry_run_uses_modified_window_counts(): void
    {
        ...
        $result = app(WordPressImportService::class)->import([
            'days' => 3,
            'basis' => 'modified',
            'import_users' => false,
