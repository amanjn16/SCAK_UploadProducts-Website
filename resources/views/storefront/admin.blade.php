@extends('layouts.app', ['hideCustomerPrompt' => true])
        ...
        <aside class="admin-drawer panel">
            ...
            <div class="admin-drawer-links admin-drawer-links-secondary">
                ...
                <form method="POST" action="{{ route('admin.logout') }}">
                    @csrf
                    <button class="admin-drawer-link" type="submit" style="width: 100%;">Log Out</button>
                </form>
            </div>
        </aside>
    </div>
    ...
    <script>
        (() => {
            ...
            const state = {
                ...
                tabs: ['products', 'stock', ...(@json($isSuperAdmin) ? ['duplicates', 'productFields'] : [])],
                products: [],
            ...
            async function loadProducts(page = 1) {
                ...
                if (state.productType) {
                    params.set('product_type', state.productType);
                }
            ...
            async function loadStockSuppliers() {
                const response = await api('/admin/stock-feedback/suppliers');
                state.stockSuppliers = response.data;
            }

            async function loadProductFields() {
                const response = await api('/admin/settings/product-fields');
                state.productFields = response.data;
            }

            async function loadDuplicates() {
                const response = await api('/admin/duplicate-products');
                state.duplicateGroups = response.data;
            }

            function renderDuplicates() {
                elements.content.innerHTML = `
                    <div class="admin-toolbar">
                        <p class="admin-meta">Only byte-for-byte identical uploaded photos are listed. Choose the product to keep. Merge archives the other product after moving its unique photos and linked records.</p>
                        <div class="admin-actions"><button class="btn-secondary" id="refreshDuplicatesButton">Refresh results</button></div>
                    </div>
                    ${state.duplicateGroups.length ? `<div class="admin-list">${state.duplicateGroups.map((group) => `
                        <article class="admin-card">
                            <div class="admin-meta">Exact image fingerprint ${escapeHtml(group.sha256.substring(0, 12))}...</div>
                            <div class="admin-grid products">${group.products.map((product) => `
                                <div class="admin-card compact">
                                    <img src="${escapeHtml(product.matching_image_url)}" alt="${escapeHtml(product.name)}" loading="lazy">
                                    <strong>${escapeHtml(product.name)}</strong>
                                    <div class="admin-meta">${escapeHtml(product.sku || '-')} · ${escapeHtml(product.brand || 'No brand')} · ${formatCurrency(product.price)}</div>
                                    <div class="admin-meta">${escapeHtml(product.product_type)} · ${product.image_count} photo(s)</div>
                                    <div class="admin-actions">
                                        ${group.products.filter((other) => other.id !== product.id).map((other) => `<button class="btn-primary duplicate-merge-button" data-keep-id="${product.id}" data-merge-id="${other.id}" data-hash="${group.sha256}">Keep this; merge ${escapeHtml(other.sku || other.name)}</button>`).join('')}
                                        <button class="btn-secondary duplicate-delete-button" data-product-id="${product.id}">Archive this</button>
                                    </div>
                                </div>`).join('')}
                            </div>
                        </article>`).join('')}</div>` : '<div class="admin-empty">No exact-image duplicate products found.</div>'}
                `;
            }

            function renderProductFields() {
                elements.content.innerHTML = `<div class="admin-card"><h2>Product fields</h2><div class="admin-meta">Hidden fields retain their saved data. Mandatory applies only when that field is enabled for the product type.</div></div>
                    <div class="admin-list">${state.productFields.map((field) => `<article class="admin-card compact">
                        <h3 class="admin-card-title">${escapeHtml(field.label)}</h3>
                        <div class="admin-meta">${escapeHtml(field.field_key)}</div>
                        <div class="admin-tag-grid">
                            ${fieldToggle(field, 'enabled_regular', 'Regular enabled')}${fieldToggle(field, 'required_regular', 'Regular mandatory')}
                            ${fieldToggle(field, 'enabled_sale', 'Sale enabled')}${fieldToggle(field, 'required_sale', 'Sale mandatory')}
                            ${fieldToggle(field, 'show_customer', 'Show to customer')}${fieldToggle(field, 'show_exports', 'Allow in PDF/sharing')}
                        </div>
                    </article>`).join('')}</div>`;
            }

            function fieldToggle(field, key, label) {
                return `<label class="admin-selection"><input type="checkbox" class="product-field-toggle" data-field-id="${field.id}" data-field-key="${key}" ${field[key] ? 'checked' : ''}> ${label}</label>`;
            }

            function renderStockFeedback() {
                if (state.stockSession) {
                    const session = state.stockSession;
                    elements.content.innerHTML = `
                        <div class="admin-toolbar"><button class="btn-secondary" id="stockBackButton">Back to suppliers</button><button class="btn-primary" id="stockAddItemButton">+ New item</button></div>
                        <div class="admin-card"><h2>${escapeHtml(session.supplier.name)}</h2><div class="admin-meta">${escapeHtml(session.count_date)} · ${escapeHtml(session.status)}</div></div>
                        <div class="admin-grid">${session.items.map((entry) => {
                            const item = entry.stock_item;
                            const photo = item.current_photo?.url;
                            return `<article class="admin-card">
                                ${photo ? `<img class="admin-card-image" src="${escapeHtml(photo)}" alt="${escapeHtml(item.name)}">` : ''}
                                <h3 class="admin-card-title">${escapeHtml(item.name)}</h3>
                                <div class="admin-meta">Yesterday: ${entry.previous_quantity} · ${escapeHtml(entry.check_status.replaceAll('_', ' '))}</div>
                                <div class="admin-actions">
                                    <button class="btn-primary stock-action-button" data-entry-id="${entry.id}" data-action="same">Same</button>
                                    <button class="btn-secondary stock-action-button" data-entry-id="${entry.id}" data-action="zero">Zero</button>
                                    <button class="btn-secondary stock-change-button" data-entry-id="${entry.id}" data-current="${entry.quantity ?? entry.previous_quantity}">Change</button>
                                    <button class="btn-secondary stock-action-button" data-entry-id="${entry.id}" data-action="not_found">Not Found</button>
                                </div>
                            </article>`;
                        }).join('')}</div>
                        <div class="admin-actions"><button class="btn-primary" id="stockSubmitButton">Submit today's stock</button></div>`;
                    return;
                }

                elements.content.innerHTML = `
                    <div class="admin-card"><h2>Choose supplier</h2><div class="admin-meta">Yesterday's item names, photos and quantities will be reused automatically.</div></div>
                    <div class="admin-grid">${state.stockSuppliers.map((supplier) => `
                        <article class="admin-card">
                            <h3 class="admin-card-title">${escapeHtml(supplier.name)}</h3>
                            <div class="admin-meta">${supplier.stock_items_count} reusable items</div>
                            <button class="btn-primary stock-start-button" data-supplier-id="${supplier.id}">Start today's stock</button>
                        </article>`).join('') || '<div class="admin-card">No suppliers are available yet.</div>'}</div>`;
            }
                ...
                elements.content.innerHTML = `
                    <div class="admin-toolbar">
                        <div class="admin-toolbar-row">
                            <button class="${state.productType === '' ? 'btn-primary' : 'btn-secondary'} product-type-button" data-product-type="">All</button>
                            <button class="${state.productType === 'regular' ? 'btn-primary' : 'btn-secondary'} product-type-button" data-product-type="regular">Regular</button>
                            <button class="${state.productType === 'sale' ? 'btn-primary' : 'btn-secondary'} product-type-button" data-product-type="sale">Sale</button>
                        </div>
                        <div class="admin-toolbar-row">
                            <div class="field">
                            ...
                            ${state.products.map((product) => `
                                <article class="admin-card compact">
                                    <div class="admin-card-head">
                                        ...
                                        <div class="admin-meta">${product.product_type === 'sale' ? 'Sale · Regular catalogue' : 'Regular'} · ${escapeHtml(product.brand || 'No brand')} · ${product.availability === 'out_of_stock' ? 'Out of stock' : 'In stock'}</div>
                            ...
                            <div class="admin-card">
                                <div class="admin-modal-grid">
                                    ...
                                    <div class="field">
                                        ...
                                        <input id="adminPinInput" type="password" inputmode="numeric" maxlength="6" placeholder="6 digits">
                                    </div>
                                    <div class="field">
                                        <label>Role</label>
            ...
            async function openProductEditor(productId = null) {
                try {
                    if (!state.productFields.length) await loadProductFields();
                    let product = null;
                    if (productId) {
            ...
            function renderProductEditorModal(product) {
                ...
                const type = product?.product_type || 'regular';
                const configurable = (key, html) => {
                    const field = state.productFields.find((item) => item.field_key === key);
                    if (!field || (!field.enabled_regular && !field.enabled_sale)) return '';
                    return `<div class="configurable-field" data-enabled-regular="${field.enabled_regular ? '1' : '0'}" data-enabled-sale="${field.enabled_sale ? '1' : '0'}" ${((type === 'sale' && field.enabled_sale) || (type === 'regular' && field.enabled_regular)) ? '' : 'hidden'}>${html}</div>`;
                };
                ...
                return `
                    ...
                    <div class="admin-modal-grid">
                        <div class="field">
                            <label>Name</label>
                            <input id="productNameInput" value="${escapeHtml(product?.name || '')}" placeholder="Enter the item name" autofocus>
                        </div>
                        <div class="field">
                            <label>Product type</label>
                            <select id="productTypeInput"><option value="regular" ${product?.product_type !== 'sale' ? 'selected' : ''}>Regular</option><option value="sale" ${product?.product_type === 'sale' ? 'selected' : ''}>Sale</option></select>
                        </div>
                        <div class="field">
                            <label>Brand</label>
                            <input id="productBrandInput" value="${escapeHtml(product?.brand || '')}">
                        </div>
                        <div class="field">
                            <label>Rate</label>
                        ...
                        <div class="field">
                            <label>In / Out Stock</label>
                            <select id="productAvailabilityInput"><option value="in_stock" ${product?.availability !== 'out_of_stock' ? 'selected' : ''}>In Stock</option><option value="out_of_stock" ${product?.availability === 'out_of_stock' ? 'selected' : ''}>Out of Stock</option></select>
                        </div>
                        ${configurable('brand_price', `<div class="field">
                            <label>Brand Price (optional)</label>
                            <input id="productBrandPriceInput" value="${escapeHtml(product?.brand_price || '')}" type="number" min="0" step="0.01">
                        </div>`)}
                        ${configurable('supplier', `<div class="field"><label>Supplier</label><input id="productSupplierInput" value="${escapeHtml(product?.supplier || '')}"></div>`)}
                        ${configurable('city', `<div class="field"><label>Supplier City</label><input id="productCityInput" value="${escapeHtml(product?.city || '')}"></div>`)}
                        ${configurable('category', `<div class="field"><label>Category</label><input id="productCategoryInput" value="${escapeHtml(product?.category || '')}"></div>`)}
                        ${configurable('tags', `<div class="field full">
                            <label>Tags (comma separated)</label>
                            <input id="productTagsInput" value="${escapeHtml((product?.tags || []).join(', '))}">
                        </div>`)}
                        ${configurable('remarks', `<div class="field full">
                        ...
                        <div class="field full">
                            ...
                            <div class="admin-actions">
                                <button class="btn-secondary" id="productSearchWebButton" type="button">Search images</button>
                                <button class="btn-secondary" id="productPasteImagesButton" type="button">Paste from clipboard</button>
                            </div>
                            <input id="productImagesInput" type="file" accept="image/*" multiple>
                            <div class="admin-meta" id="productImageSelectionHint">Choose, drag, or paste up to 36 photos. Existing Sale app clipboard behavior is supported.</div>
                        </div>
                        <div class="field full">
                            <label>Paste image addresses</label>
                            <textarea id="productImageUrlsInput" rows="2" placeholder="Paste one or more https:// image links, separated by spaces or new lines"></textarea>
                        </div>
                        <div class="field full">
            ...
            function parseImageUrls(value) {
                return Array.from(new Set((String(value).match(/https?:\/\/[^\s,]+/gi) || []).map((url) => url.trim())));
            }

            function addFilesToProductInput(files) {
                const input = document.getElementById('productImagesInput');
                if (!input) return;
                const transfer = new DataTransfer();
                Array.from(input.files || []).forEach((file) => transfer.items.add(file));
                Array.from(files || []).filter((file) => file?.type?.startsWith('image/')).forEach((file) => transfer.items.add(file));
                if (transfer.files.length > 36) throw new Error('A product can have at most 36 images.');
                input.files = transfer.files;
                const hint = document.getElementById('productImageSelectionHint');
                if (hint) hint.textContent = `${transfer.files.length} photo${transfer.files.length === 1 ? '' : 's'} ready to upload.`;
            }

            async function pasteProductImages() {
                if (!navigator.clipboard?.read) throw new Error('Clipboard image access is unavailable in this browser. Use Choose files or paste image links below.');
                const files = [];
                const urls = [];
                for (const item of await navigator.clipboard.read()) {
                    for (const type of item.types) {
                        if (type.startsWith('image/')) {
                            ...
                            urls.push(...parseImageUrls(await (await item.getType(type)).text()));
                        }
                    }
                }
                if (files.length) addFilesToProductInput(files);
                if (urls.length) {
                    const box = document.getElementById('productImageUrlsInput');
                    box.value = Array.from(new Set([...parseImageUrls(box.value), ...urls])).join('\n');
                }
                if (!files.length && !urls.length) throw new Error('No image or image link was found on the clipboard.');
                showStatus(`${files.length} clipboard photo${files.length === 1 ? '' : 's'} and ${urls.length} link${urls.length === 1 ? '' : 's'} ready.`);
            }
            ...
            async function saveCurrentProduct() {
                ...
                const payload = {
                    ...
                    brand_price: document.getElementById('productBrandPriceInput')?.value ? Number(document.getElementById('productBrandPriceInput').value) : null,
                    supplier: document.getElementById('productSupplierInput')?.value.trim() || null,
                    city: document.getElementById('productCityInput')?.value.trim() || null,
                    category: document.getElementById('productCategoryInput')?.value.trim() || null,
                    availability: document.getElementById('productAvailabilityInput').value,
                ...
                const pastedUrls = parseImageUrls(document.getElementById('productImageUrlsInput')?.value || '');
                if (pastedUrls.length) {
                    await api(`/admin/products/${savedProduct.id}/images/url`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ urls: pastedUrls }),
                    });
                }
            ...
            document.addEventListener('click', async (event) => {
                ...
                if (event.target.id === 'refreshDuplicatesButton') {
                    await loadDuplicates();
                    renderDuplicates();
                    showStatus('Duplicate results refreshed.');
                    return;
                }

                const mergeDuplicateButton = event.target.closest('.duplicate-merge-button');
                if (mergeDuplicateButton) {
                    if (!confirm('Merge these products? The product you did not choose will be archived, not permanently erased.')) return;
                    await api('/admin/duplicate-products/merge', {
                        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
                            keep_product_id: Number(mergeDuplicateButton.dataset.keepId),
                            merge_product_id: Number(mergeDuplicateButton.dataset.mergeId),
                            sha256: mergeDuplicateButton.dataset.hash,
                        }),
                    });
                    await loadDuplicates();
                    renderDuplicates();
                    showStatus('Products merged; the duplicate was archived.');
                    return;
                }

                const deleteDuplicateButton = event.target.closest('.duplicate-delete-button');
                if (deleteDuplicateButton) {
                    if (!confirm('Archive this duplicate product? It remains recoverable.')) return;
                    await api(`/admin/duplicate-products/${deleteDuplicateButton.dataset.productId}`, { method: 'DELETE' });
                    await loadDuplicates();
                    renderDuplicates();
                    showStatus('Duplicate product archived.');
                    return;
                }
                ...
                const stockStartButton = event.target.closest('.stock-start-button');
                if (stockStartButton) {
                    const response = await api(`/admin/stock-feedback/suppliers/${stockStartButton.dataset.supplierId}/sessions`, {
                        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}),
                    });
                    state.stockSession = response.data;
                    renderStockFeedback();
                    return;
                }

                const stockActionButton = event.target.closest('.stock-action-button');
                if (stockActionButton && state.stockSession) {
                    await api(`/admin/stock-feedback/sessions/${state.stockSession.id}/items/${stockActionButton.dataset.entryId}`, {
                        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: stockActionButton.dataset.action }),
                    });
                    const response = await api(`/admin/stock-feedback/suppliers/${state.stockSession.supplier.id}/sessions`, {
                        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ count_date: state.stockSession.count_date.substring(0, 10) }),
                    });
                    state.stockSession = response.data;
                    renderStockFeedback();
                    return;
                ...
                const stockChangeButton = event.target.closest('.stock-change-button');
                if (stockChangeButton && state.stockSession) {
                    const raw = prompt('Enter current quantity', stockChangeButton.dataset.current);
                    if (raw === null) return;
                    const quantity = Number(raw);
                    if (!Number.isInteger(quantity) || quantity < 0) throw new Error('Enter a whole quantity of zero or more.');
                    await api(`/admin/stock-feedback/sessions/${state.stockSession.id}/items/${stockChangeButton.dataset.entryId}`, {
                        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'change', quantity }),
                    });
                    const response = await api(`/admin/stock-feedback/suppliers/${state.stockSession.supplier.id}/sessions`, {
                        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ count_date: state.stockSession.count_date.substring(0, 10) }),
                    });
                ...
                if (event.target.id === 'stockSubmitButton' && state.stockSession) {
                    await api(`/admin/stock-feedback/sessions/${state.stockSession.id}/submit`, { method: 'POST' });
                    showStatus('Today\'s stock feedback submitted.');
                ...
                if (event.target.id === 'stockAddItemButton' && state.stockSession) {
                    openModal(`<div class="admin-modal-header"><h2>New stock item</h2><button class="btn-secondary" id="closeAdminModalButton">Close</button></div>
                        <div class="admin-modal-grid">
                            <div class="field full"><label>Item name</label><input id="stockNewNameInput" autocomplete="off"></div>
                            <div class="field"><label>Current quantity</label><input id="stockNewQuantityInput" type="number" min="0" step="1" value="0"></div>
                            <div class="field"><label>Bundle photo</label><input id="stockNewPhotoInput" type="file" accept="image/*" capture="environment"></div>
                        </div><div class="admin-actions"><button class="btn-primary" id="stockSaveNewItemButton">Save & add to daily list</button></div>`);
                    return;
                }

                if (event.target.id === 'stockSaveNewItemButton' && state.stockSession) {
                    const name = document.getElementById('stockNewNameInput').value.trim();
                    const quantity = Number(document.getElementById('stockNewQuantityInput').value);
                    if (!name) throw new Error('Item name is required.');
                    if (!Number.isInteger(quantity) || quantity < 0) throw new Error('Enter a whole quantity of zero or more.');
                    const response = await api(`/admin/stock-feedback/sessions/${state.stockSession.id}/items`, {
                        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, quantity }),
                    });
                    state.stockSession = response.data;
                    const newEntry = state.stockSession.items.find((entry) => entry.stock_item.name.toLocaleLowerCase() === name.toLocaleLowerCase());
                    const photo = document.getElementById('stockNewPhotoInput').files[0];
                    if (photo && newEntry) {
                        const form = new FormData();
                        form.append('photo', photo); form.append('kind', 'bundle');
                        await api(`/admin/stock-feedback/items/${newEntry.stock_item.id}/photos`, { method: 'POST', body: form });
                        const refreshed = await api(`/admin/stock-feedback/suppliers/${state.stockSession.supplier.id}/sessions`, {
                            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ count_date: state.stockSession.count_date.substring(0, 10) }),
                        });
                        state.stockSession = refreshed.data;
                    }
                    closeModal();
                    renderStockFeedback();
                    return;
                ...
                if (event.target.id === 'stockBackButton') {
                    state.stockSession = null;
                    await loadStockSuppliers();
                    renderStockFeedback();
                    return;
                ...
                const productTypeButton = event.target.closest('.product-type-button');
                if (productTypeButton) {
                    state.productType = productTypeButton.dataset.productType;
                    await loadProducts();
                    renderProducts();
                    return;
                }
                ...
                if (event.target.id === 'productSearchWebButton') {
                    const query = document.getElementById('productNameInput')?.value.trim();
                    if (!query) throw new Error('Enter the item name before searching.');
                    window.open(`https://www.google.com/search?tbm=isch&q=${encodeURIComponent(query)}`, '_blank', 'noopener');
                    return;
                }

                if (event.target.id === 'productPasteImagesButton') {
                    try {
                        await pasteProductImages();
                    } catch (error) {
                        showStatus(error.message, true);
                    }
                    return;
                ...
                if (event.target.id === 'saveAdminButton') {
                    await api('/admin/admin-users', {
                        ...
                        body: JSON.stringify({
                            ...
                            pin: document.getElementById('adminPinInput').value,
                            is_active: true,
                        }),
                    });
            ...
            document.addEventListener('paste', (event) => {
                if (!elements.modal.contains(event.target)) return;
                const files = Array.from(event.clipboardData?.files || []).filter((file) => file.type.startsWith('image/'));
                if (!files.length) return;
                event.preventDefault();
                try {
                    addFilesToProductInput(files);
                    showStatus(`${files.length} pasted photo${files.length === 1 ? '' : 's'} ready to upload.`);
                } catch (error) {
                    showStatus(error.message, true);
                }
            });
            ...
            document.addEventListener('change', async (event) => {
                if (event.target.id === 'productImagesInput') {
                    try {
                        if (event.target.files.length > 36) throw new Error('A product can have at most 36 images.');
                        const hint = document.getElementById('productImageSelectionHint');
                        if (hint) hint.textContent = `${event.target.files.length} photo${event.target.files.length === 1 ? '' : 's'} ready to upload.`;
                    } catch (error) {
                        showStatus(error.message, true);
                    }
                    return;
                }
                if (event.target.id === 'productTypeInput') {
                    document.querySelectorAll('.configurable-field').forEach((field) => {
                        field.hidden = field.dataset[event.target.value === 'sale' ? 'enabledSale' : 'enabledRegular'] !== '1';
                    });
                }
                const fieldToggle = event.target.closest('.product-field-toggle');
                if (fieldToggle) {
                    const field = state.productFields.find((item) => item.id === Number(fieldToggle.dataset.fieldId));
                    if (!field) return;
                    const key = fieldToggle.dataset.fieldKey;
                    await api(`/admin/settings/product-fields/${field.id}`, {
                        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ [key]: fieldToggle.checked }),
                    });
                    field[key] = fieldToggle.checked;
                    if (key.startsWith('enabled_') && !fieldToggle.checked) {
                        const requiredKey = key.replace('enabled_', 'required_');
                        if (field[requiredKey]) {
                            await api(`/admin/settings/product-fields/${field.id}`, {
                                method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ [requiredKey]: false }),
                            });
                            field[requiredKey] = false;
                        }
                    }
                    showStatus(`${field.label} setting saved.`);
                    renderProductFields();
                }
            });
