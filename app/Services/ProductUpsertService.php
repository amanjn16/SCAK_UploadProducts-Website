...
class ProductUpsertService
{
    ...
    public function upsert(array $payload, ?Product $product = null): Product
    {
        ...
        $product->fill([
            'name' => $normalizedName,
            'product_type' => $payload['product_type'] ?? $product->product_type ?? 'regular',
            'brand' => $this->normalizeDisplayName($payload['brand'] ?? $product->brand ?? ''),
            ...
            'price' => $payload['price'],
            'brand_price' => $payload['brand_price'] ?? null,
            'availability' => $payload['availability'] ?? $product->availability ?? 'in_stock',
    ...
    protected function storeProductImageBinary(Product $product, array $optimized, ?string $preferredPath = null): array
    {
        ...
        return [
            ...
            'sha256' => hash('sha256', $optimized['binary']),
        ];
    }
